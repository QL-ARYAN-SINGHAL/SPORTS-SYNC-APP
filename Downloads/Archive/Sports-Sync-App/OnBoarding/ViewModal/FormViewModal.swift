...
class FormViewModal: ObservableObject {
    ...
    func validateSignUpData() -> Bool {
        ...
        if let signupEmail = data.signUpEmail, signupEmail.trimmingCharacters(in: .whitespaces).isEmpty {
        ...
        if let firstName = data.firstName , firstName.trimmingCharacters(in: .whitespaces).isEmpty {
            alertMessage = "First Name is required."
            return false
        }
        if  let firstName = data.firstName, firstName.rangeOfCharacter(from: nameCharacterSet.inverted)
            != nil
        {
            alertMessage = "First Name contains invalid characters."
            return false
        }
        if let lastName = data.lastName, lastName.trimmingCharacters(in: .whitespaces).isEmpty {
            alertMessage = "Last Name is required."
            return false
        }
        if let lastName = data.lastName, lastName.rangeOfCharacter(from: nameCharacterSet.inverted)
            != nil
        {
            alertMessage = "Last Name contains invalid characters."
            return false
        }
        if let password = data.signUpPassword, password.isEmpty {
    ...
    func calculateProgress(from signUpData: SignUpDataModel) -> Int {
        ...
        DispatchQueue.main.async {
            ...
            if let firstName = signUpData.firstName, !firstName.isEmpty {
                newProgress += 17
            }
            if let lastName = signUpData.lastName, !lastName.isEmpty {
                newProgress += 17
            }
            if let signUpEmail  = signUpData.signUpEmail ,!signUpEmail.isEmpty {
                newProgress += 17
            }
            if let signUpPassword = signUpData.signUpPassword, !signUpPassword.isEmpty,  let confirmPassword =  signUpData.confirmPassword, !confirmPassword.isEmpty
            {
                newProgress += 17
            }
